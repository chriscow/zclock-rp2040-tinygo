...
// nearlyEqual compares two float32s and returns whether they are equal, accounting for rounding errors.At worst, the
...
func nearlyEqual(a, b float32) bool {
	...
	if a == 0.0 || b == 0.0 || diff < math.SmallestNonzeroFloat32 {
	...
	return diff/float32(math.Min(absA+absB, math.MaxFloat32)) < epsilon
}
...
func (u Vec) Scaled(c float32) Vec {
...
func (u Vec) Cross(v Vec) float32 {
...
func (u Vec) Map(f func(float32) float32) Vec {
	return Vec{
		f(u.X),
		f(u.Y),
	}
...
func (l Line) Closest(v Vec) Vec {
	...
	between := func(a, b, x float32) bool {
		min := math.Min(a, b)
		max := math.Max(a, b)
		return min < x && x < max
	}
...
func (l Line) Formula() (m, b float32) {
	// Account for horizontal lines
	if l.B.Y == l.A.Y {
		return 0, l.A.Y
	}
...
func (l Line) Rotated(around Vec, angle float32) Line {
...
func C(center Vec, radius float32) Circle {
	return Circle{
		Center: center,
		Radius: radius,
	}
...
func (c Circle) Resized(radiusDelta float32) Circle {
	return Circle{
		Center: c.Center,
...
func (c Circle) Formula() (h, k float32) {
...
func (m Matrix) Scaled(around Vec, scale float32) Matrix {
	return m.ScaledXY(around, V(scale, scale))
}

// Rotated rotates everything around a given point by the given angle in radians.
func (m Matrix) Rotated(around Vec, angle float32) Matrix {
